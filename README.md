...
커스텀 D3D11 엔진, 커널 레벨 가상 오디오 드라이버, 그리고 AMD SVM 하이퍼바이저의
...
* WASAPI 직결 렌더러: OS 믹서 오버헤드를 원천 차단하는 제로 레이턴시 가상 오디오 드라이버 구현
...
* Data-Oriented Design (SoA) : 메모리 아레나를 구축하여 동적 할당 오버헤드 차단.
...
* **VM Exit/Entry 제어**: 순수 x64 어셈블리를 활용한 Context Switching 및 MSR 제어.
